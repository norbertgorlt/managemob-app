...
const EMPTY: Partial<MobilityServiceProvider> = { name: '', contact_person: '', email: '', phone: '', city: '', country: '' }
...
export default function MobilityProvidersPage() {
  ...
  const filtered = items.filter(p => p.name.toLowerCase().includes(search.toLowerCase()) || (p.country || '').toLowerCase().includes(search.toLowerCase()))
